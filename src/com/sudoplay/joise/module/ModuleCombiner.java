...
public class ModuleCombiner extends Module {
  ...
  protected double screenGet(double x, double y) {
    double value = 1.0;
    for (int i = 0; i < MAX_SOURCES; i++) {
      if (sources[i] != null) value *= (1.0 - sources[i].get(x, y));
    }
    return 1.0 - value;
  }

  protected double screenGet(double x, double y, double z) {
    double value = 1.0;
    for (int i = 0; i < MAX_SOURCES; i++) {
      if (sources[i] != null) value *= (1.0 - sources[i].get(x, y, z));
    }
    return 1.0 - value;
  }

  protected double screenGet(double x, double y, double z, double w) {
    double value = 1.0;
    for (int i = 0; i < MAX_SOURCES; i++) {
      if (sources[i] != null) value *= (1.0 - sources[i].get(x, y, z, w));
    }
    return 1.0 - value;
  }

  protected double screenGet(double x, double y, double z, double w, double u, double v) {
    double value = 1.0;
    for (int i = 0; i < MAX_SOURCES; i++) {
      if (sources[i] != null) value *= (1.0 - sources[i].get(x, y, z, w, u, v));
    }
    return 1.0 - value;
  }
